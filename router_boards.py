...
async def add_todo_list_on_board(
    ...
    to_do_list_new_item = await create_task(to_do_list_id, text, session)
...
async def add_todo_list_item_on_board(
    ...
    to_do_list_new_item = await create_task(int(to_do_list_id), text, session)
    return {"to_do_list_id": to_do_list_id, "to_do_list_new_item" : to_do_list_new_item }

@router.post("/main_page/{board_id}/update_to_do_list_item")
async def update_todo_list_item_on_board(
    board_id: str,
    data: Dict = Body(...),
    session: AsyncSession = Depends(get_session)
):
    ...
    await update_task(int(to_do_list_id),int(to_do_list_item_id), text, completed, session)
    return {"status": "success"}
